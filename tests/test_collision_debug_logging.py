...
class TestCollisionDebugLogging:
    ...
    def test_debug_logging_with_projectiles_and_zombies(self, game_engine, caplog):
        ...
        projectile = Projectile(
            position=Vector2(90, 100), direction=Vector2(1, 0), damage=10
        )
        ...
        collision_result_logs = [
            msg for msg in log_messages if "COLLISION RESULT" in msg
        ]
    ...
    def test_debug_logging_with_no_zombies(self, game_engine, caplog):
        ...
        assert (
            len(collision_result_logs) > 0
        ), "Should log collision result even with no zombies"
...
class TestEnhancedCollisionDebugLogging:
    ...
    def test_debug_logging_always_runs_when_projectiles_exist(
        self, game_engine, caplog
    ):
        ...
        collision_check_logs = [msg for msg in log_messages if "COLLISION CHECK" in msg]
        assert (
            len(collision_check_logs) > 0
        ), "Should log collision check when projectiles exist"

        # Verify log shows 0 visible zombies
        assert any("0 visible zombies" in msg for msg in collision_check_logs)

    def test_debug_logging_shows_filtering_details_when_all_filtered(
        self, game_engine, caplog
    ):
        ...
        log_messages = [record.message for record in caplog.records]
        filter_warnings = [
            msg for msg in log_messages if "ALL ZOMBIES FILTERED OUT" in msg
        ]
    ...
    def test_debug_logging_shows_hidden_flag_in_zombie_details(
        self, game_engine, caplog
    ):
    ...
    def test_debug_logging_counts_offscreen_zombies_separately(
        self, game_engine, caplog
    ):
        ...
        log_messages = [record.message for record in caplog.records]
        breakdown_logs = [
            msg for msg in log_messages if "Hidden:" in msg and "Off-screen:" in msg
        ]
    ...
    def test_debug_logging_no_warning_when_some_zombies_visible(
        self, game_engine, caplog
    ):
        ...
        ), "Should not warn when some zombies are visible"
