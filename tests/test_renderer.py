...
class TestAuditorRendering:
    ...
    def test_render_auditor_draws_on_screen(
        self, renderer, mock_game_map, sample_auditor
    ):
    ...
    def test_render_auditor_not_drawn_when_off_screen(
        self, renderer, mock_game_map, sample_auditor
    ):
    ...
    def test_render_auditor_uses_correct_colors(
        self, renderer, mock_game_map, sample_auditor
    ):
        ...
        with patch("pygame.draw.rect") as mock_rect, patch(
            ...
            rect_calls = [
                call[0][1] for call in mock_rect.call_args_list
            ]  # Get color arguments

            # Should have dark colors for suit (20-30 RGB range)
            dark_colors = [
                color
                for color in rect_calls
                if isinstance(color, tuple) and max(color) <= 50
            ]
            ...
            skin_colors = [
                color
                for color in circle_calls
                if isinstance(color, tuple) and color[0] > 200
            ]
            assert len(skin_colors) > 0, "Should use skin tone for head and hands"

    def test_render_auditor_draws_clipboard(
        self, renderer, mock_game_map, sample_auditor
    ):
        ...
        with patch("pygame.draw.rect") as mock_rect, patch(
            ...
            assert (
                mock_line.call_count >= 3
            ), "Should draw at least 3 lines for checklist"

    def test_render_auditor_draws_sunglasses(
        self, renderer, mock_game_map, sample_auditor
    ):
        ...
        with patch("pygame.draw.rect") as mock_rect:
            ...
            assert (
                len(black_colors) >= 2
            ), "Should have at least 2 black rectangles for sunglasses"


class TestAdminRoleRendering:
    """Tests for admin role rendering (gold suit with crown)."""

    def test_render_admin_roles_draws_on_screen(
        self, renderer, mock_game_map, sample_admin_role
    ):
        """Test that admin roles are rendered when on screen."""
        with patch("pygame.draw.rect") as mock_rect, patch(
            "pygame.draw.circle"
        ) as mock_circle, patch("pygame.draw.polygon") as mock_polygon:

            renderer.render_admin_roles(
                [sample_admin_role], mock_game_map, pulse_time=0.0
            )
    ...
    def test_render_admin_roles_uses_gold_color_when_unprotected(
        self, renderer, mock_game_map, sample_admin_role
    ):
        ...
        with patch("pygame.draw.rect") as mock_rect:
            ...
            assert (
                len(gold_colors) > 0
            ), "Should use gold color for unprotected admin role"

    def test_render_admin_roles_uses_green_color_when_protected(
        self, renderer, mock_game_map, sample_admin_role
    ):
        ...
        with patch("pygame.draw.rect") as mock_rect:
            ...
            assert (
                len(green_colors) > 0
            ), "Should use green color for protected admin role"

    def test_render_admin_roles_draws_crown(
        self, renderer, mock_game_map, sample_admin_role
    ):
    ...
    def test_render_admin_roles_draws_shield_when_protected(
        self, renderer, mock_game_map, sample_admin_role
    ):
        ...
        with patch("shield.render_shield") as mock_render_shield:
            renderer.render_admin_roles(
                [sample_admin_role], mock_game_map, pulse_time=0.5
            )
            ...
            assert (
                call_args[0][1] == sample_admin_role
            ), "Should pass admin role to shield renderer"

    def test_render_admin_roles_no_shield_when_unprotected(
        self, renderer, mock_game_map, sample_admin_role
    ):
    ...
    def test_render_admin_roles_draws_label(
        self, renderer, mock_game_map, sample_admin_role
    ):
        ...
        try:
            ...
            assert any(
                "AdministratorAccess" in str(call) for call in render_calls
            ), "Should render permission set name as label"
        finally:
            renderer.small_font = original_font

    def test_render_admin_roles_not_drawn_when_off_screen(
        self, renderer, mock_game_map, sample_admin_role
    ):
