...
class TestZombieStuckCollisionBug:
    ...
    def test_projectile_hits_zombie_with_correct_state(
    ...
    def test_projectile_passes_through_zombie_with_quarantining_flag(
        self, mock_pygame, mock_api_client
    ):
        ...
        zombie.is_quarantining = (
            True  # BUG: This flag is set but zombie is still in list
        )
        zombie.is_hidden = False
    ...
    def test_player_stuck_between_zombies_with_quarantining_flags(
        self, mock_pygame, mock_api_client
    ):
        """
        ...
        # Player shoots right
        projectile = Projectile(
            position=Vector2(105, 400), direction=Vector2(1, 0), damage=10
        )
        engine.projectiles = [projectile]

        # Update collisions
        collisions = check_collisions_with_spatial_grid(
            engine.projectiles, engine.zombies, engine.spatial_grid
        )
    ...
    def test_api_failure_leaves_zombie_in_bad_state(self, mock_pygame, mock_api_client):
        ...
        mock_api_client.quarantine_identity.return_value = QuarantineResult(
            success=False, identity_id="zombie-1", error_message="API Error"
        )
    ...
    def test_zombie_state_after_quest_completion(self, mock_pygame, mock_api_client):
        ...
        projectile = Projectile(
            position=Vector2(95, 400), direction=Vector2(1, 0), damage=10
        )

        collisions = check_collisions_with_spatial_grid(
            [projectile], engine.zombies, engine.spatial_grid
        )
...
class TestZombieStateResetFix:
    ...
    def test_quest_completion_resets_zombie_quarantining_flags(
        self, mock_pygame, mock_api_client
    ):
        """
    ...
    def test_api_error_handler_restores_zombie_state(
        self, mock_pygame, mock_api_client
    ):
        """
