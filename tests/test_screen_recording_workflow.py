...
def mock_api_client():
    ...
    client.fetch_jit_configuration.return_value = {"enrolledPermissionSets": []}

    # Mock JIT protection application
    client.apply_jit_protection.return_value = QuarantineResult(
        success=True, identity_id="ps-admin-001", error_message=None
    )
    ...
    client.quarantine_identity.return_value = QuarantineResult(
        success=True, identity_id="test-identity"
    )
...
def game_engine(mock_api_client, mock_level_manager):
    ...
    account_data = {"577945324761": 100, "613056517323": 50}
...
class TestScreenRecordingWorkflow:
    ...
    def test_3_hacker_challenge_initialization(self, game_engine):
        ...
        sandbox_door = next(
            (
                d
                for d in game_engine.game_map.doors
                if d.destination_room_name == "MyHealth - Sandbox"
            ),
            None,
        )
        game_engine._enter_level(sandbox_door)
        ...
        if quest:
            ...
            print(
                "✅ Test 3 PASSED: Hacker challenge initialized (service unprotected)"
            )
        else:
    ...
    def test_9_enter_production_level(self, game_engine):
        ...
        production_door = next(
            (
                d
                for d in game_engine.game_map.doors
                if d.destination_room_name == "MyHealth - Production"
            ),
            None,
        )
    ...
    def test_10_jit_quest_initialization(self, game_engine):
        ...
        assert (
            game_engine.game_state.jit_quest.protected_count == 0
        )  # None protected initially
