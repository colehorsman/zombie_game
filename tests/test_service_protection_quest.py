...
class TestServiceProtectionQuestZombieStateBugFix:
    ...
    def test_zombie_quarantine_flags_reset_on_quest_success(
        self, game_engine_with_zombies
    ):
        """
    ...
    def test_zombie_state_correct_when_no_incorrect_flags(
        self, game_engine_with_zombies
    ):
        """
    ...
    def test_zombie_count_logged_correctly_on_quest_success(
        self, game_engine_with_zombies, caplog
    ):
        """
        ...
        zombie_count_log = [
            log for log in debug_logs if "zombies in list" in log.message
        ]
    ...
    def test_api_error_does_not_trigger_zombie_reset(self, game_engine_with_zombies):
        ...
        engine.api_client.protect_service.return_value = QuarantineResult(
            success=False, identity_id="", error_message="API Error"
        )
...
class TestServiceProtectionQuestIntegration:
    ...
    def test_complete_quest_workflow_with_zombie_state_fix(
        self, game_engine_with_zombies
    ):
        """
        ...
        service_node = create_service_node(
            service_type="bedrock-agentcore", position=Vector2(5000, 400)
        )
